...
## データを関数fnに渡す前に正規化を適用。勾配消失を防ぎ、学習の安定化を図る
...
## 入力データをチャンクに分割し、各チャンクごとに処理を行う。大規模データの効率的処理に適用
...
## データの次元を変換してから関数fnを適用し、次元を必要であれば戻す。次元数の調整に使う
...
## 時系列データの前方または、後方へのシフトを実現。データ変換やトークン関係性の強化に使用
...
## 入力データをセグメントごとに分割し、異なるシフトを適用してから関数を実行
...
## トランスフォーマー層で、トークンごとの特徴を変換するフィードフォワードネットワークを実装
...
#       加速度データのためここは削除
        ...
        self.token_emb = nn.Linear(300, emb_dim)
        ...
        # # self.predict = nn.Linear(emb_dim, 300)
